...
# =========================================================
# --- 로봇 동작 모드 ---
#   False (기존/배달 모드): 항상 컵을 향해 이동하며 사람을 피해 우회.
#                          DANGER면 정지(SPEED_SCALE_BY_RISK["DANGER"]=0).
#   True  (대기/회피 모드): 평소엔 제자리에 정지. 인력이 없으므로 아무것도
#                          가깝지 않으면 합력이 0 -> 자연히 가만히 있는다.
#                          사람 손이 위험하게 접근하면(WARN/DANGER) 척력만으로
#                          그 반대 방향으로 물러난다 - 이게 이 모드의 "회피"다.
#   ★ True일 때는 DANGER에서도 절대 SPEED_SCALE_BY_RISK로 속도를 죽이지 않는다
#     (planning/potential_field.py, main.py 참고) - 죽이면 회피 동작 자체가 안 나온다.
#     대신 PF_HUMAN_GAIN_BY_RISK가 위험도에 따라 밀어내는 힘을 키워 긴급도를 표현한다.
IDLE_UNTIL_THREAT = True

# --- 인력 (목표 컵) ---
# IDLE_UNTIL_THREAT=True면 아래 인력 파라미터는 사용되지 않는다.
...
# 8) 전송 계층 - UDP
# =========================================================
ESP32_IP = "10.182.7.50"    # ESP32의 IP (고정 IP 권장)
ESP32_PORT = 8888
